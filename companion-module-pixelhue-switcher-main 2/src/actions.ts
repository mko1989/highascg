		...
		loadPreset: {
			...
			callback: async (event) => {
				try {
					...
					if (event.options.loadIn === -1) {
						const presetScreenGuids = preset.screens.map((screen) => screen.guid)
						const screensToTake = self.screens.filter((screen) => presetScreenGuids.includes(screen.guid))

						if (screensToTake.length > 0) {
							await self.apiClient?.loadPreset(preset, LoadIn.preview)
							await self.apiClient?.take(screensToTake, self.swapEnabled, false, self.effectTime)
						}
					} else {
