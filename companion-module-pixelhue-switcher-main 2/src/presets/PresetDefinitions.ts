...
export function getPresetsPresetDefinitions(self: ModuleInstance): CompanionPresetDefinitions {
	...
	self.presets.forEach((preset) => {
		...
		/* Load in Program (Auto-Take) */
		presetsPresetDefinitions['presetLabelProgramAuto'] = {
			type: 'text',
			category: PRESET_CATEGORY.PROGRAM_AUTO,
			text: 'Load selected preset in Program (Auto-Take)',
			name: 'Program (Auto-Take)',
		}
		presetsPresetDefinitions[`preset${preset.serial}ProgramAuto`] = {
			type: 'button',
			name: `Preset ${preset.serial} Load (Program Auto-Take)`,
			category: PRESET_CATEGORY.PROGRAM_AUTO,
			style: {
				text: `$(${self.id}:preset_${preset.serial}_name)`,
				size: 16,
