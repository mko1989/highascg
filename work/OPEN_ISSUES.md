...
| CONFIG-APPLY-CLOBBER | Full apply at 2026-07-15 15:42 (live UI POST) rewrote config/*.json losing fields: streamKey blanked, screen_3_operator_monitor + multiview_always_on_top flipped false, multiview_os_rate/quality/layout keys dropped — apply payload doesn't round-trip saved-only fields; old values recoverable from git (`git diff config/`). Needs a WO | — | 🔴 Open (left uncommitted on purpose) |
