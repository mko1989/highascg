...
- 2026-07-14 12:51 (orchestrator, post-restart capture) — Owner reports the crop mismatch now on the PGM cell. Evidence gathered: (a) composite `ch1.jpg` snapshot (scratchpad wo190-ch1-125155.jpg) SHOWS the crop applied (windowed content + border) → the channel output is correct; (b) MV cells re-applied at 12:44:35 with whole-channel routes + contain fills (log excerpt in WO); (c) owner observation "reset and rebuild fixes it" ⇒ consistent with **H1 stale/doubled cells** (Caspar itself has not restarted; pre-lock-era applies could have left cells). With the WO-190 apply lock + WO-201 chain fix now RUNNING, new stale cells shouldn't form — next mismatch: hit Refresh output; if it cures it again, H1 is confirmed and the residual fix is a startup/one-time MV channel sweep. NOTE: `GET /api/multiview/debug` was hit by the SAME router-registration gap as WO-188/189/193 (fifth instance) — registered now (fixed arg shape too); serves after the NEXT service restart.
