...
- 2026-07-15 — FIX-3 (2026-07-15 review, work/reviews/2026-07-15-take-pipeline.md finding 2): the previewOnly branch in src/api/routes-scene-take.js (~line 172-181) passed `forceCut: !!b.forceCut` instead of hardcoding `true` like the other two banklessTake preview call sites — PRV staging is always a cut and WO-209's banklessTake design assumes it. Changed to `forceCut: true` with an explanatory comment; not reachable from the shipped client today but closes the gap for direct API callers. No smoke test extension requested for this finding; verified via node --check/eslint and the offline CI gate (202/202 non-skipped pass).
