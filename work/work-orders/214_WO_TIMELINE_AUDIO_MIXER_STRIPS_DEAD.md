...
- 2026-07-15 — FIX-4 (2026-07-15 review, work/reviews/2026-07-15-infra-misc.md finding 3): `getActiveTimelineForChannel` (client/lib/audio-mixer-rows.js:75) did `sendTo.screenIdx ?? 0`, collapsing the server's legitimate ALL-SCREENS sentinel (`screenIdx: null`/`'all'`, see `normalizeTimelineSendTo` in src/engine/timeline-playback-helpers.js:156-160) to screen 0 — every other program channel silently lost its timeline mixer strips when a timeline was sent to "All". Fixed: when screenIdx is null/'all', the timeline now matches ANY program channel via `cm.programChannels.map(Number).includes(Number(channel))`; the normal indexed case is unchanged. Extended tools/smoke/smoke-wo214-timeline-mixer-rows.test.js with a source-grep check for the new branch plus a behavioral unit test (falls back to source-grep-only coverage if the ESM module can't be imported standalone) — all tests pass.
