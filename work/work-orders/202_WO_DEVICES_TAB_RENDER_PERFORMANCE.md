...
- 2026-07-14 (orchestrator hotfixes after owner report "loading devices… never loads"):
  1. **Skeleton regression fixed** — the T202.1 skeleton used `refs.layout.innerHTML = …`, which DETACHED the shell's pre-built mount nodes (destBody/bands/matrix captured at init); renderFromState then wrote into orphaned elements and the pane never left the loading state. Replaced with a non-destructive appended overlay, removed in a `finally` around the fetch (also covers the error path).
  2. **304 path was dead code** — the If-None-Match check read `query`, not request headers (the T202.5 "304 verified by curl" claim was inaccurate; live revalidation returned 200). Router registration now passes `req` and the handler reads `req.headers['if-none-match']`. Verified: node --check + eslint + device-view smokes green. Both fixes need the service restart + reload like the rest of the batch.
