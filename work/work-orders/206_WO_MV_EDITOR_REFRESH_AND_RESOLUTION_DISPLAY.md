...
- 2026-07-15 — FIX-1 (2026-07-15 review, work/reviews/2026-07-15-multiview.md finding 1): auto-apply could fire mid-drag with unfinished cell geometry (setCell() emitted 'apply-request' on every mousemove with no drag-in-progress check). Added `multiviewState.dragInProgress` + `setDragInProgress(v)` (client/lib/multiview-state.js); the editor's mousedown (drag start) sets it true, mouseup/mouseleave (drag end) set it false (client/components/multiview-editor.js); the 'apply-request' listener now skips scheduling while dragging, and drag-end fires exactly one final apply-request. Verified via new source-grep smoke tools/smoke/smoke-wo206-mv-drag-guard.test.js (5/5 pass).
