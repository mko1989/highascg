...
- 2026-07-15 — FIX-5 (2026-07-15 review, work/reviews/2026-07-15-timers.md finding 3): "pause shows configured duration instead of frozen remaining" (owner-reported 3×). Implemented server-authoritative freeze: src/api/routes-screen-timers.js handleCmd now computes elapsed-adjusted remainingSec BEFORE recording a pause and stores it via new src/engine/screen-timers.js `recordTimerPause(timerId, remainingSec)`; `recordTimerCmd` clears remainingSec on 'reset'. A 'start' after a pause includes `{cmd:'start', remainingSec}` in the CG UPDATE payload; template/countdown/countdown-engine.js's `doStart()`/`applyConfig()` now accept and honor an explicit resume value (backward compatible — omitted is a no-op, exactly today's local-paused-state behavior). Client client/components/timer-control-panel.js `computeDisplayTime` now uses `record.remainingSec` for both the paused display and the post-resume running display, falling back to config.durationSec when absent. GET /api/timers/list already round-trips remainingSec via listScreenTimers' deep copy — no change needed there. Extended tools/smoke/smoke-wo210-screen-timers.test.js with a recordTimerPause round-trip test (persistence, clamping, reset-clears, start-preserves) — 21/21 pass.
