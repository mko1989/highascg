...
[dev-push.sh](dev-push.sh) — server-only **`tar`** → **`ssh`** upload → extract into **`DEPLOY_PATH`** (keeps remote `highascg.config.json`, `node_modules`, `config/`). Operator UI: [**highascg-client**](https://github.com/mko1989/highascg-client).
