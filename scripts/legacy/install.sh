...
for _f in install-phase1.sh install-phase2.sh install-phase3.sh install-phase4.sh install-phase5.sh; do
...
for _f in install-config.sh install-helpers.sh; do
	if [[ ! -f "$SCRIPT_DIR/scripts/lib/$_f" ]]; then
		echo "Error: missing $SCRIPT_DIR/scripts/lib/$_f — copy the whole scripts/ directory from the repo." >&2
		exit 1
	fi
done

# shellcheck source=../lib/install-config.sh
source "$SCRIPT_DIR/scripts/lib/install-config.sh"
