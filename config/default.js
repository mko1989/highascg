...
module.exports = {
	 ...
	 * `configPath`: main server XML (separate from media-scanner config under `/opt/casparcg`). Env `CASPAR_CONFIG_PATH` overrides at apply time.
	 */
	casparServer: {
		screen_count: 1,
	 ...
	 * When set, `led_grid_test.html` and its eye SVGs are copied here so they are **not** placed under
	...
	/**
	 * Audio routing (Settings → Audio / OSC). Consumed by config-generator via merge into flat Caspar keys.
	 */
	audioRouting: {
