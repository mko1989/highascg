/*
 * ShaderToyLite.js — vendored from https://github.com/chipweinberger/ShaderToyLite.js
 * Upstream commit 11bbc09ac0e78791895e9a022e27cc749d7e27c5 (2025-05-23), BSD-3-Clause
 * (license: vendor/shadertoy/LICENSE; pristine snapshot: vendor/shadertoy/ShaderToyLite.js).
 *
 * LOCAL PATCH (WO-266): setShader accepts custom iChannel keys (e.g. 'audio') registered via
 * addTexture(), not only 'A'-'D' — upstream exposes addTexture but gives no way to bind the
 * texture to a channel. Single-condition change, marked "WO-266 patch" below.
 */
    ...
    var setShader = (config, key) => {
        if (config) {
            ...
            for (let i = 0; i < 4; i++) {
                ...
                // WO-266 patch: also accept custom texture keys already registered via
                // addTexture() — call addTexture BEFORE setImage/setBufferX.
                if (s == "A" || s == "B" || s == "C" || s == "D" || (typeof s === "string" && s in atexture)) {
                    ichannels[key][i] = s;
                }
            }
        } else {
