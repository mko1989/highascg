...
describe('host-operator-fullscreen', () => {
	...
	it('resolveOperatorRouteTarget uses interactive multiview when configured', (t) => {
		const config = loadProjectConfig(extraLiveSources)
		const target = resolveOperatorRouteTarget(config)
		if (!target) {
			t.skip('no interactive zones in project config')
			return
		}
	...
	it('buildOperatorFullscreenState exposes cefFocusTarget for WO-89', () => {
		...
		assert.equal(state.cefFocusTarget.sourceId, 'webpage_test')
		assert.equal(state.cefFocusTarget.hostChannel, 20)
		assert.equal(state.cefFocusTarget.needle, 'interactive_click_test')
		assert.equal(state.cefFocusTarget.zoneId, 'multiview')
	})
	...
	it('toggle off returns host-still-running message', async () => {
		...
		assert.equal(ctx._cefFocusTarget, null)
	})
	...
	it('apply sends PLAY route on operator layer', async (t) => {
		...
		assert.equal(ctx._cefFocusTarget?.hostChannel, 20)
	})
})
