		...
		ws.on('message', async (raw) => {
			try {
				...
				if (msg.type === 'amcp' && msg.cmd) {
					...
					// WO-334 (rev 2026-07-26): a payload identical to this client's previous one
					// WITHIN a short window is a storm echo — drop it before merge/persist/broadcast.
					// Windowed + only cached after a SUCCESSFUL merge: the first version cached
					// unconditionally and forever, which swallowed a remote client's legitimate
					// retries whenever its earlier merge was guard-rejected ("blocking laptop edits").
					const rawDeck = JSON.stringify(msg.data)
					if (ws._lastDeckSyncJson === rawDeck && Date.now() - (ws._lastDeckSyncAt || 0) < 2000) {
						return
					}
					...
					if (merged) {
						...
						ws._lastDeckSyncAt = Date.now()
					} else {
						ws._lastDeckSyncJson = null
					}
					if (merged) {
						try {
