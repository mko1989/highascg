 ...
 * 6) Bank swap / pointer flip / orphan sweep / timer visibility: scene-take-lbg-bank-finalize.js.
...
const { finalizeTakeBankState } = require('./scene-take-lbg-bank-finalize')
...
async function runSceneTakeLbg(amcp, opts) {
	...
	const { takeJobs, extraExitCandidates, timelineFadeInPhys, skippedVisuallyEqualLayers } = await buildTakeJobs({
		...
		outgoingTopIsTimeline: !!activeTimelineIdToFadeOut,
	})
	...
	await finalizeTakeBankState(amcp, {
		self,
		channel,
		chKey,
		incoming,
		incomingSceneOpt: opts?.incomingScene,
		takeJobs,
		mergeMixerExtras,
		isMergeTransition,
		banklessTake,
