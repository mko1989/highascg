 ...
 * Body: { channel, layer, overlay: { type, params }, fill: { x, y, scaleX, scaleY } }
 */
async function handleApply(body, ctx) {
	...
	const lines = buildPipOverlayAmcpLines(overlay, channel, layer, fill, ctx)
	if (lines.length === 0) {
...
async function handleUpdate(body, ctx) {
	...
	const lines = buildPipOverlayUpdateLines(channel, layer, overlay, fill, ctx)
	try {
		await sendPipOverlayLinesSerial(ctx.amcp, lines)
		await ctx.amcp.mixerCommit(channel)
		return { status: 200, headers: JSON_HEADERS, body: jsonBody({ ok: true }) }
...
async function handleRemove(body, ctx) {
	...
	const lines = buildPipOverlayRemoveLines(channel, layer)
	try {
		await sendPipOverlayLinesSerial(ctx.amcp, lines)
		return { status: 200, headers: JSON_HEADERS, body: jsonBody({ ok: true }) }
	} catch (e) {
