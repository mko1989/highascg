...
export function createTimelineTransport(deps) {
	...
	function buildTransport() {
		...
		transportEl.querySelector('#tl-select')?.addEventListener('change', async (e) => {
			const prev = timelineState.getActive()
			if (prev) timelineState.setSendTo(prev.id, view.sendTo)
