...
export function appendCasparRearPanelMarkers({
	...
	markerItems.forEach((it) => {
		...
		if ((it.kind === 'decklink_out' || it.kind === 'decklink_io') && live?.decklink?.outputs) {
			...
			if (dlOut && !dlOut.ok) {
				marker.title = `${marker.title} — ${dlOut.reason || 'Invalid SDI output resolution'}`
				marker.classList.add('device-view__panel-marker--decklink-invalid')
			} else if (dlOut?.inherited?.standardModeId) {
				const inh = dlOut.inherited
				marker.title = `${marker.title} — inherits ${inh.standardModeId} (${inh.width}×${inh.height} @ ${inh.fps} Hz)`
			}
		}
