...
export async function mountSystemSettings(container) {
	...
	try {
		...
		container.innerHTML = `
			...
			<div class="settings-group">
				<label>Default system audio output (${hwA.cached ? 'cached' : 'fresh'})</label>
				<select id="sys-audio-device" style="width: 100%; margin-bottom: 0.5rem;">
				...
				<p class="settings-note">Choice is saved in HighAsCG config. On save, the default is written to this user’s <code>~/.asoundrc</code> (no sudo; applies to Caspar/FFmpeg run as the same user). For machine-wide <code>/etc/asound.conf</code>, call <code>POST /api/audio/default-device</code> with JSON <code>{"scope":"system"}</code> plus card/device (requires root or passwordless sudo).</p>
			</div>
